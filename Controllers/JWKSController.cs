...
public class JWKSController : ControllerBase
{
    ...
    public IActionResult GetJWKS()
    {
        ...
        var keys = _jwtDbContext.SigningKeys.Where(k => k.IsActive).ToList();
        ...
        // Return the JWKS object as a JSON response with status code 200
        return Ok(jwks);
    }
    
    // Helper method to extract the modulus component from a Base64-encoded public key
    public byte[] GetModulus(string publicKey)
    {
        ...
        {
            throw new InvalidOperationException("RSA Operation are not valid");
        }
